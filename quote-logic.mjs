...
import { recordQuoteCompletion } from "./lib/stats-store.mjs";
...
export async function handleQuoteRequestCreate(body, siteUrl) {
  ...
  if (promoCode) {
    ...
    try {
      ...
      await recordQuoteCompletion({ paidVia: "promo", jobType: pending.estimate?.jobType || null });
...
export async function handleQuoteFinalize({ requestId, sessionId }) {
  ...
  try {
    ...
    await recordQuoteCompletion({
      paidVia: "stripe",
      amountCents: session.amount_total ?? QUOTE_PRICE_CENTS,
      jobType: pending.estimate?.jobType || null,
    });
