...
export const defaultMonetizationConfig = {
  ...
  googleAnalyticsId: "",
  // Footer social icons. Blank url = that icon never renders, even though
  // the slot exists — no code change needed once Dan has a profile to add.
  socialLinks: {
...
export async function getMonetizationConfig() {
  ...
  return {
    ...
    socialLinks: {
      ...structuredClone(defaultMonetizationConfig.socialLinks),
      ...(stored.socialLinks || {}),
    },
  };
}
