...
//
// LIVE PRICING: the AI is given the web_search tool, capped at
// 3 uses per request, and instructed to look up real current
// prices for the 2-3 priciest/most significant materials at
// Home Depot or Lowe's. Those real prices are used for BOTH the
// material cost total AND the suggested_products shown with
// affiliate links, so the two numbers always match.
// ============================================================
...
// Cap on how many live web searches the AI can do per estimate.
// Each search costs ~$0.01 on top of token costs — keep this small.
const MAX_WEB_SEARCHES_PER_ESTIMATE = 3;
...
You have a web_search tool available, limited to ${MAX_WEB_SEARCHES_PER_ESTIMATE} uses. Use it to look up REAL CURRENT prices at Home Depot or Lowe's for the 2-3 most significant/expensive materials this job needs (skip searching for cheap incidentals like screws or tape). Use your own knowledge for everything else. When you do look up a price, use that real number — do not round it down.

For every item in suggested_products, include the price you found (or your best conservative retail estimate if you didn't search for that particular item). The material cost total you report in each task should be built from these same real prices, so the estimate and the suggested products always agree with each other.

Underestimating materials is worse than overestimating, so when unsure of an exact price, round up rather than down.
...
suggested_products should have 2-5 specific, genuinely useful items (e.g. "18v cordless drill", "5-gallon joint compound") — not vague categories like "tools". IMPORTANT: after any tool use, your FINAL message must be ONLY the JSON object above — no prose before or after it.`;
...
async function callAnthropic({ description, photoDataUrls }) {
  ...
  // Web search means Claude may need a couple of extra round trips, so
  // give it more time than a plain single-shot call.
  const timeout = setTimeout(() => controller.abort(), pricingConfig.aiTimeoutMs * 2);
  ...
  try {
        ...
        tools: [
          {
            ...
            name: "web_search",
            max_uses: MAX_WEB_SEARCHES_PER_ESTIMATE,
          },
        ],
      }),
    ...
    // With web_search enabled, the response can contain multiple content
    // blocks (tool_use, tool_result, text) rather than one plain text
    // block. We only want the model's final text — collect every text
    // block and use the LAST one, since that's Claude's final answer
    // after it's done searching.
...
function validateAndNormalize(raw) {
  ...
  return {
    ...
    tasks,
    suggestedProducts: normalizeSuggestedProducts(raw.suggested_products),
...
// suggested_products now carries a real price per item instead of just a
// name. Still accepts plain strings too (older AI responses, or the
// fallback path) so nothing downstream breaks if price is missing.
function normalizeSuggestedProducts(raw) {
  ...
  return raw
    ...
    .map((p) => {
      if (typeof p === "string") {
        return { name: p.slice(0, 80), price: null, pricedFromSearch: false };
      }
      if (p && typeof p === "object") {
        const name = String(p.name || "").slice(0, 80);
        if (!name) return null;
        const price = clampNumber(p.price, 0, 10000);
        return {
          name,
          price: Number.isFinite(price) && p.price != null ? price : null,
          pricedFromSearch: Boolean(p.priced_from_search),
        };
      }
      return null;
    })
...
function fallbackAnalysis(description) {
  ...
  // No AI call in the fallback path, so we can't look up real prices —
  // just offer the matched job names themselves as a starting search term,
  // with no price attached (the UI shows "See price" instead of a number).
  const suggestedProducts =
    matched.length > 0 ? matched.slice(0, 5).map((j) => ({ name: j.name, price: null, pricedFromSearch: false })) : [];
