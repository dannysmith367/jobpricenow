...
// The admin ad-slot field is usually just filled in with a bare AdSense
// slot ID number (e.g. "9499716496"), same as the main app's ad slots.
// Since these blog pages are server-rendered (no client JS to build the
// real embed the way app.js does), build it here instead — a bare number
// becomes a real AdSense unit; anything else (a full snippet from AdSense,
// Ezoic, or a direct sponsor) is assumed to already be real markup and
// passed through as-is.
function buildAdSlotMarkup(raw) {
  const trimmed = String(raw || "").trim();
  if (!trimmed) return "";
  if (/^\d+$/.test(trimmed)) {
    return `<ins class="adsbygoogle" style="display:block" data-ad-client="${ADSENSE_CLIENT_ID}" data-ad-slot="${trimmed}" data-ad-format="auto" data-full-width-responsive="true"></ins><script>(adsbygoogle = window.adsbygoogle || []).push({});</script>`;
  }
  return trimmed;
}
...
<script>window.dataLayer=window.dataLayer||[];function gtag(){dataLayer.push(arguments);}gtag('js',new Date());gtag('config','${escapeHtml(gaId)}');</script>`
    : "";
  const adSenseHtml = adSlotHtml && adSlotHtml.includes("adsbygoogle")
...
<header class="b-header">
  <div class="b-wrap b-header-inner">
    ...
    <nav class="b-nav">
      <a href="/pricing" style="color:rgba(255,255,255,0.75);text-decoration:none;font-size:13.5px;font-weight:700;">Pricing Guides</a>
      <a href="/blog" class="b-nav-blog">Blog</a>
      <a href="/" class="b-nav-cta">Get a price →</a>
      <button type="button" class="nav-toggle" id="nav-toggle" aria-label="Open menu" aria-expanded="false" style="background:rgba(255,255,255,0.12);border-color:rgba(255,255,255,0.25);color:#fff;">
...
<nav class="site-nav" id="site-nav" aria-label="Site menu">
  <div class="site-nav-header">
    <span class="brand-word" style="font-size:16px;color:var(--navy);">JobPrice<span class="accent">Now</span></span>
...
<footer class="b-footer">
  <div class="b-wrap b-footer-inner">
    ...
    <nav class="b-footer-nav" style="margin-top:8px;">
      <a href="/about.html" style="color:inherit;opacity:0.85;margin-right:14px;">About</a>
      <a href="/contact.html" style="color:inherit;opacity:0.85;">Contact</a>
    </nav>
  </div>
</footer>
</body>
...
export function renderBlogIndex(posts, monetization) {
  const adHtml = monetization?.adsEnabled ? buildAdSlotMarkup(monetization?.adSlots?.blog_ad_slot?.html) : "";
