...
const defaultSummary = {
  ...
  totalPwaInstalls: 0,
  totalPwaLaunches: 0,
  daily: {}, // "YYYY-MM-DD": { estimates, quotesPaid, quotesComped, revenueCents, pwaInstalls, pwaLaunches }
};
...
export async function recordEstimate({ jobType, zip, persona, priceLow, priceHigh } = {}) {
  try {
    ...
    summary.daily[day] = summary.daily[day] || { estimates: 0, quotesPaid: 0, quotesComped: 0, revenueCents: 0, pwaInstalls: 0, pwaLaunches: 0 };
...
/**
 * Call this when a visitor installs the PWA (Android/Chrome 'appinstalled'
 * event) or opens it in standalone/home-screen mode (detected on load, both
 * Android and iOS). `type` distinguishes the two so the admin panel can
 * show real install counts separately from repeat-open activity — installs
 * are Android/Chrome-only since iOS Safari doesn't expose an install event,
 * platform is recorded so that gap is visible rather than silently guessed at.
 */
export async function recordPwaEvent({ type, platform } = {}) {
  try {
    if (type !== "install" && type !== "launch") return;
    const now = new Date();
    const day = todayKey(now);
    ...
    if (type === "install") {
      summary.totalPwaInstalls += 1;
      summary.daily[day].pwaInstalls += 1;
    } else {
      summary.totalPwaLaunches += 1;
      summary.daily[day].pwaLaunches += 1;
    }
    summary.daily = trimOldDailyEntries(summary.daily);
    await kvSet(SUMMARY_KEY, summary);
  } catch (err) {
    console.error("stats: failed to record PWA event (non-fatal)", err);
  }
}
...
export async function getStatsSummary({ dailyDays = 30 } = {}) {
  ...
  return {
    ...
    totalPwaInstalls: summary.totalPwaInstalls,
    totalPwaLaunches: summary.totalPwaLaunches,
