...
export async function generateQuotePdf(quote) {
  ...
  drawText(c.businessName || c.contractorName || "Handyman Quote", { size: 20, f: bold, gap: 26 });
  if (c.contractorName && c.businessName) drawText(c.contractorName, { size: 11, color: MUTED, gap: 14 });
  const contactLine = [c.phone, c.email, c.website].filter(Boolean).join("   •   ");
  if (contactLine) drawText(contactLine, { size: 10, color: MUTED, gap: 12 });
  if (c.address) drawText(c.address, { size: 10, color: MUTED, gap: 12 });
  if (c.licenseNumber) drawText(`License #${c.licenseNumber}`, { size: 10, color: MUTED, gap: 12 });
