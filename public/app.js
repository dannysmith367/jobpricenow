(() => {
  ...
  els.form.addEventListener("submit", async (e) => {
    ...
    lastRequestBody = { description, zip, photos: photos.map((p) => p.dataUrl), persona: getPersona() };
  ...
  function renderResult(data) {
    ...
    // The two highest-value actions — turning this into a paid, branded
    // quote, and shopping the materials — sit right under the price where
    // they can't be missed, instead of buried below the stats and details.
    html += `
    ...
    // Right after someone gets real value from the tool is the best moment
    // to offer quick access next time — much stronger than hoping they
    // notice the small header button on their own. Only renders when the
    // browser has actually signaled the site is installable, and never
    // reappears once dismissed for this visit.
    if (deferredInstallPrompt && !installPromptDismissedThisSession) {
      html += `
        <div class="install-prompt-box" id="post-estimate-install-prompt">
          <span class="install-prompt-text">Add JobPriceNow to your home screen for one-tap access next time.</span>
          <div class="install-prompt-actions">
            <button type="button" class="install-prompt-btn" id="post-estimate-install-btn">Install</button>
            <button type="button" class="install-prompt-dismiss" id="post-estimate-install-dismiss" aria-label="Dismiss">&times;</button>
          </div>
        </div>
      `;
    }
  ...
  function renderProQuoteCard() {
    return `
      <div class="card pro-quote-box">
        <p class="pro-quote-eyebrow">Look professional. Win the job.</p>
        <ul class="pro-quote-list">
          <li>Branded PDF, ready to send</li>
          <li>Email or text it to your customer</li>
          <li>No account, no subscription — just $2.99</li>
        </ul>
  ...
  function renderMaterialsCard() {
    // The Pro view always shows material product suggestions — a contractor
    // doesn't need a "find a pro" link, they are the pro. Only the Homeowner
    // view respects the homeownerMaterialsMode toggle from admin.
    const isHomeowner = getPersona() === "homeowner";
    if (isHomeowner && siteConfig?.homeownerMaterialsMode === "findAPro") {
      return renderFindAProBox();
    }
  ...
  function renderFindAProBox() {
    // Provider-agnostic: whichever partner is set as "active" in admin is
    // the one shown here. Falls back to nothing if that partner isn't
    // actually turned on/filled in, so a mid-switch or unapproved program
    // never shows a broken or empty CTA.
    const partnerKey = siteConfig?.activeLeadGenPartner;
    const partner = siteConfig?.leadGenPartners?.[partnerKey];
    if (!partner?.enabled || !partner?.urlTemplate) return "";
    const zip = els.zip?.value?.trim() || "";
    const url = partner.urlTemplate.replace("{ZIP}", encodeURIComponent(zip));
    return `
      <div class="materials-box" style="text-align:center;">
        ...
        <a href="${url}" target="_blank" rel="noopener sponsored" class="btn-primary btn-full find-a-pro-link" data-partner="${escapeHtmlAttr(partnerKey)}" style="display:inline-block;text-decoration:none;margin-top:8px;">Find a Pro on ${escapeHtmlAttr(partner.label)} →</a>
      </div>
    `;
  }

  // Delegated click tracking for the Find a Pro link — the box gets
  // re-rendered on every estimate, so a delegated listener on a stable
  // ancestor is more reliable than re-binding after each render.
  document.addEventListener("click", (e) => {
    const findAProLink = e.target.closest?.(".find-a-pro-link");
    if (findAProLink) track("lead_gen_click", { partner: findAProLink.dataset.partner });
    const materialLink = e.target.closest?.(".material-affiliate-link");
    if (materialLink) track("material_affiliate_click", { partner: materialLink.dataset.partner });

    const toggleBtn = e.target.closest?.(".affiliate-links-toggle");
    if (toggleBtn) {
      const container = toggleBtn.closest(".affiliate-links");
      const extra = container?.querySelector(".affiliate-links-extra");
      if (!extra) return;
      const expanded = toggleBtn.dataset.expanded === "true";
      extra.hidden = expanded;
      toggleBtn.dataset.expanded = String(!expanded);
      toggleBtn.textContent = expanded ? toggleBtn.dataset.moreLabel : toggleBtn.dataset.lessLabel;
    }
  });
  ...
  function buildAffiliateLinksHtml() {
    ...
    const rows = products.map((product) => {
      ...
      const partnerLinks = enabledPartners
        .map((p) => {
          ...
          return `<a href="${url}" target="_blank" rel="noopener sponsored" class="affiliate-link material-affiliate-link" data-partner="${escapeHtml(p.label)}">${escapeHtml(p.label)} →</a>`;
        })
        .join("");
      const priceHtml =
    ...
    // Only show the first couple of materials by default so this card stays
    // short — the stat grid (labor/materials/difficulty/confidence) sits
    // right below it and shouldn't get pushed way down the page by a long
    // product list. Extra items are one tap away via "Show more."
    const VISIBLE_COUNT = 2;
    const visibleRows = rows.slice(0, VISIBLE_COUNT);
    const extraRows = rows.slice(VISIBLE_COUNT);

    const toggleHtml = extraRows.length
      ? `
        <div class="affiliate-links-extra" hidden>${extraRows.join("")}</div>
        <button type="button" class="affiliate-links-toggle" data-expanded="false" data-more-label="Show ${extraRows.length} more ${extraRows.length === 1 ? "item" : "items"} ▾" data-less-label="Show less ▴">Show ${extraRows.length} more ${extraRows.length === 1 ? "item" : "items"} ▾</button>`
      : "";

    return `<div class="affiliate-links">${visibleRows.join("")}${toggleHtml}</div>`;
  }
  ...
  function loadAdSenseScript() {
    ...
    adSenseScriptLoading = new Promise((resolve) => {
      ...
      script.onerror = () => resolve(); // don't block ad rendering forever if the script fails to load
      document.head.appendChild(script);
    });
  ...
  function injectAdSlot(slotEl) {
    ...
    const raw = siteConfig.adSlots?.[slotKey]?.html;
    if (!raw) return;
    const trimmed = String(raw).trim();
    if (!trimmed) return;

    if (/^\d+$/.test(trimmed)) {
      // The admin field is usually just filled in with a bare AdSense slot
      // ID number — build the real ad unit from it.
      const ins = document.createElement("ins");
      ins.className = "adsbygoogle";
      ins.style.display = "block";
      ins.setAttribute("data-ad-client", ADSENSE_CLIENT_ID);
      ins.setAttribute("data-ad-slot", trimmed);
      ...
      // Already a full embed snippet (AdSense, another network, or a
      // direct sponsor tag). innerHTML alone won't execute <script> tags,
      // so any scripts are recreated and re-inserted to actually run.
      slotEl.innerHTML = trimmed;
      slotEl.classList.add("enabled");
      slotEl.querySelectorAll("script").forEach((oldScript) => {
  ...
  function wireResultInteractions(data, pricing) {
    ...
    document.getElementById("post-estimate-install-btn")?.addEventListener("click", () => {
      triggerInstallPrompt("post_estimate");
    });
    document.getElementById("post-estimate-install-dismiss")?.addEventListener("click", () => {
      installPromptDismissedThisSession = true;
      document.getElementById("post-estimate-install-prompt")?.remove();
    });
  ...
  // ---------------- PWA install & launch tracking ----------------
  // Fires both into GA4 (via the existing track() pipeline) and into the
  // site's own stats store (via /api/track-pwa), so PWA numbers show up
  // in the admin Stats tab without needing to go check Google Analytics.
  function pingPwaEvent(type, platform) {
    track(type === "install" ? "pwa_installed" : "pwa_launch_standalone", { platform });
    try {
      fetch("/api/track-pwa", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
      ...
      // Never let a tracking ping break anything else on the page.
    }
  }
  ...
  // Chrome/Android only — this event is what makes an install button
  // possible at all. It fires once the browser has decided the site is
  // installable, and gives us a prompt() we can trigger from our own UI
  // instead of relying on someone noticing the tiny address-bar icon.
  // Safari never fires this, so on iOS neither install button ever shows —
  // there's no way around that; only the manual Share > Add to Home
  // Screen path exists there.
  let deferredInstallPrompt = null;
  let installPromptDismissedThisSession = false;

  window.addEventListener("beforeinstallprompt", (e) => {
    e.preventDefault();
    deferredInstallPrompt = e;
    document.getElementById("header-install-btn")?.removeAttribute("hidden");
    // Re-render the current result (if any) so the post-estimate prompt
    // can appear retroactively — the event can fire after the estimate
    // is already on screen, not just before.
    if (lastEstimateResponse) renderResult(lastEstimateResponse);
  });

  async function triggerInstallPrompt(sourceLabel) {
    if (!deferredInstallPrompt) return;
    track("pwa_install_prompted", { source: sourceLabel });
    deferredInstallPrompt.prompt();
    const choice = await deferredInstallPrompt.userChoice.catch(() => null);
    track("pwa_install_prompt_outcome", { source: sourceLabel, outcome: choice?.outcome || "unknown" });
    // The prompt can only be used once — clear it either way so a stale
    // reference never gets called again after the browser's already done
    // with it.
    deferredInstallPrompt = null;
    document.getElementById("header-install-btn")?.setAttribute("hidden", "");
    if (lastEstimateResponse) renderResult(lastEstimateResponse);
  }

  document.getElementById("header-install-btn")?.addEventListener("click", () => triggerInstallPrompt("header"));

  // Android/Chrome fires 'appinstalled' once the visitor actually completes
  // the install — this is the real, reliable install count. iOS Safari has
  // no equivalent event at all (Apple doesn't expose one), which is why
  // platform is tracked alongside every event: it makes that coverage gap
  // visible in the stats instead of implying iOS installs are just zero.
  window.addEventListener("appinstalled", () => {
    pingPwaEvent("install", detectPwaPlatform());
    deferredInstallPrompt = null;
    document.getElementById("header-install-btn")?.setAttribute("hidden", "");
  });

  // Standalone-mode detection catches every subsequent open from a home
  // screen icon, on both Android and iOS — this is the only install signal
  // iOS gets at all, so it's the number to lean on for iPhone usage.
  const isStandalone =
    window.matchMedia?.("(display-mode: standalone)")?.matches || window.navigator?.standalone === true;
  if (isStandalone) {
    pingPwaEvent("launch", detectPwaPlatform());
  }
